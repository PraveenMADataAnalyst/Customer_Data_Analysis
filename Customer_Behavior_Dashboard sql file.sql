...
select shipping_type,round(avg(`purchase_amount_(usd)`),2) from customer where shipping_type in('Standard','Express') group by shipping_type;
...
from customer where previous_purchases>5 group by 
subscription_status;
